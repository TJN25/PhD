...
# We are only interested in the host-target pairs with more than one match (hit)
  ...
  # Write some stats for the manuscript
      Stats.Table<-Main.Data%>%group_by(hits.count)%>%summarise(Counts=n())%>%mutate(Counts=Counts/hits.count)
      write.csv(Stats.Table,paste(Out.Dir,"Analysis/PreFilterHitsCounts.csv",sep=""),row.names=F)
      Stats.Table<-Main.Data%>%group_by(bit.score.num)%>%summarise(Counts=n())
      write.csv(Stats.Table,paste(Out.Dir,"Analysis/PreFilterBitScoreAll.csv",sep=""),row.names=F)
      Stats.Table<-Main.Data%>%group_by(bit.score.num)%>%filter(hits.count>=2)%>%summarise(Counts=n())
      write.csv(Stats.Table,paste(Out.Dir,"Analysis/PreFilterBitScore2orMoreHits.csv",sep=""),row.names=F)
...
# Stats for the manuscript
    Final.Data%>%filter(spacer.order.number==1)%>%group_by(PPS.score)%>%summarise(freq=n())%>%mutate(percentage=freq/sum(freq)*100)
    
      Stats.Table<-Final.Data%>%group_by(bit.score.num)%>%filter(spacer.order.number==1)%>%summarise(PPS.bitscore=n())
      Stats.Table<-Stats.Table%>%full_join(Final.Data%>%group_by(bit.score.num)%>%filter(spacer.order.number>1)%>%summarise(nonPPS.bitscore=n()))
      write.csv(Stats.Table,paste(Out.Dir,"Analysis/FinalBitScores.csv",sep=""),row.names=F)

      Stats.Table<-Final.Data%>%group_by(hits.count)%>%summarise(Counts=n())%>%mutate(Counts=Counts/hits.count)
      write.csv(Stats.Table,paste(Out.Dir,"Analysis/FinalHitsCounts.csv",sep=""),row.names=F)
...
for(i in subtypes){ #@@ write table?
  x<-ks_test_analysisB(to.map, Rnd.Dist.Data, Subtype.label = i)
