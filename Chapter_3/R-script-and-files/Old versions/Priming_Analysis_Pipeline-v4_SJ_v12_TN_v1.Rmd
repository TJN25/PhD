...
quadrant_analysis <- function(dat, Subtype.label, use.hits = 2, window.width = 5000, run.statistical.test = F, write.data = F){

   IFData <- dat%>%
     filter(Subtype == Subtype.label)%>%
     filter(spacer.order.number >= 2)%>%
     filter(spacer.order.number <= use.hits)%>%
     filter(distance.to.protospacer > -window.width)%>%
     filter(distance.to.protospacer < window.width)
...
if(write.data ==T){
  if(run.statistical.test == F){
outputMat <- matrix(c(quadrants[c(2,4,1,3), 3], strands[c(2,1),3],  length(unique(IFData$host.target.pair)), 1), ncol = 2, byrow = T)
  }else{
outputMat <- matrix(c(quadrants[c(2,4,1,3), 3], strands[c(2,1),3],  length(unique(IFData$host.target.pair)), mat.res$p.value), ncol = 2, byrow = T)
 ...
 write.csv(outputMat, file = paste(i, "_quadrant_clustered.csv", sep = ""), row.names = FALSE)
}
...
##!!*** There is something wrong with the host.acc column in spacers - temporary fix for now ##Proper fix has been made
  ...
  aa<-spacers.in%>%select(-host.acc.)%>%unique() ##problem is related to the host.acc. column ##this has been fixed
...
swipeData<-swipeData%>%mutate(subtype.list=ifelse(!is.na(subtype.list.y),subtype.list.y,subtype.list.x))%>%select(-subtype.list.y, -subtype.list.x)
...
# Now remove cases where the repeat family and subtype call don't match
    ...
    ab<-aa%>%mutate(keep.genome = ifelse(subtype.list == Subtype & subtype.list == subtypes.genes.found, 1, ifelse(Subtype == "II" , ifelse(subtype.list == "II-A" | subtype.list == "II-B" | subtype.list == "II-C", 1, 0), 0))) ##This change is important
...
# Keep the systems with consistent subtype calls
    ...
    genomesToRemove<-ab%>%filter(keep.genome==0)
...
genomesToRemove<-genomesToRemove%>%select(-subtypes.genes.found, -subtypes.count,-keep.genome)%>%
  ungroup()%>%mutate(array.id = paste(contig, array.num, sep = "-"))%>%
        mutate(spacer.num.corrected= spacer.num)
genomesToRemove <- genomesToRemove%>%ungroup()%>%
        mutate(spacer.num.corrected= spacer.num)%>%mutate(spacer.id = paste(assembly_accession,array.id, spacer.num, sep = "-"))%>%
    select(spacer.id, Direction, Subtype, spacer.num.corrected,array.length)
...
genomesToRemove<-genomesToRemove%>%mutate(to.keep=0)

allGenomes <- genomesToKeep%>%bind_rows(genomesToRemove)
...
#**** Swipedata comes back into play here
      ...
      swipeDataFixed<-swipeDataFixed%>%left_join(allGenomes, by = "spacer.id")
    ...
    # Still correcting for cases not in spacer file...        
      swipeDataFixed$to.keep[is.na(swipeDataFixed$to.keep)]<-1
...
Counts <- Counts%>%filter(!is.na(FailedRepeatType))
...
swipeData<-swipeData%>%
    group_by(assembly_accession,array.id)%>%mutate(array.length= ifelse(is.na(array.length), max(spacer.num), array.length))
...
swipeData.in <-  read.table("refseq_83_swipe_array_direction_corrected.txt", comment.char = "", fill = T, sep = "\t", header = T, quote = "", as.is=T)
swipeData.old <-  read.table("Old/refseq_83_swipe_array_direction_corrected.txt", comment.char = "", fill = T, sep = "\t", header = T, quote = "", as.is=T)

swipeData.in%>%group_by(host.target.pair)%>%mutate(hits.count=n())%>%group_by(hits.count)%>%summarise(freq=n()) ##!!?? why do we have only single hits in here??!! ## I can't see any...
...
swipeData <- unique(swipeData.in) ##not needed
...
#!!!!!!!!!! The read.table function needs to have quote = "" and comment.char = "" or there will be lines that are missed !!!!!!!!!!#
swipeData.in <-  read.table("refseq_83_swipe_host.target.pair.setup.txt", comment.char = "", quote = "", fill = T, sep = "\t", header = T,as.is=T)
swipeData.old <-  read.table("Old/refseq_83_swipe_host.target.pair.setup.txt", comment.char = "", quote = "", fill = T, sep = "\t", header = T,as.is=T)

swipeData.in%>%group_by(host.target.pair)%>%mutate(hits.count=n())%>%group_by(hits.count)%>%summarise(freq=n()) ##!!?? why do we have only single hits in here??!! ##See above comment

Counts<-Counts%>%full_join(swipeData.in%>%group_by(subtype.list)%>%summarise(Section4In=n()))

##get the number of arrays involved in each host-target pair
number.of.arrays <- swipeData.in%>%group_by(host.target.pair)%>%summarise(num.of.arrays= length(unique(array.id)))

##add a column containing the number of arrays in each host.target.pair
swipeData <- left_join(swipeData.in, number.of.arrays, by = "host.target.pair")
...
swipeData%>%group_by(host.target.pair)%>%mutate(hits.count=n())%>%group_by(hits.count)%>%summarise(freq=n()) ##!!?? why do we have only single hits in here??!! ## I can't see any here either
...
first.and.second.spacer.dat <- mutate(first.and.second.spacer.dat, spacer.diff.num = spacer.num.2 - spacer.number) ##changed so that it is an absolute difference

##if the percentage difference is 65% then score the PPS 3 and keep the hits with a PPS score 3 or greater
first.and.second.spacer.dat <-  mutate(first.and.second.spacer.dat, PPS.score.num = ifelse(spacer.diff.num < -10, 3, 0))%>%filter(PPS.score.num ==3)%>%select(-spacer.diff.num, -spacer.num.2) ##changed the cutoff for scoring a 3 to the difference being more that 10 spacers
...
aa%>%group_by(PPS.score.num)%>%summarise(counts=n())
...
swipeData.in <-  read.table("refseq_83_swipe_PPS.scores.txt", comment.char = "", quote = "", fill = T, sep = "\t", header = T,as.is=T)
swipeData.old <-  read.table("Old/refseq_83_swipe_PPS.scores.txt", comment.char = "", quote = "", fill = T, sep = "\t", header = T,as.is=T)

# Check host-target pairs have at least 2 hits
  swipeData <- swipeData.in%>%group_by(host.target.pair)%>%mutate(hits.count = n()) 
  table(swipeData$hits.count) #*******!!Check this*********
...
# Filter for only the subtypes we are interested in.
    ...
    Counts <- Counts%>%filter(!is.na(RawSwipeData))
...
#** Additional checks for similar hosts
  ...
  # Seperate any host-target pairs with more than 1 shared spacer on the same target
    ...
    # Working on this section - need to fix up so it writes out the removed data + adds counts
      # check if there are duplicated PS within the same host-target pair
      aa<-many.shared.PS.not.dblPPS%>%group_by(host.target.pair)%>%mutate(UNQ.PS.hits=length(unique(unique.protospacer.host.match)))%>%
        mutate(to.keep=ifelse(UNQ.PS.hits==hits.count,1,0))

       ab<-aa%>%filter(to.keep==0)
        # write.csv(ab,paste(Out.Dir,"Discard/multipleIdentPS.csv",sep=""))
        Counts<-Counts%>%full_join(ab%>%group_by(subtype.list)%>%summarise(multipleIdentPS=n()),by="subtype.list")
      
        ac<-aa%>%filter(to.keep==1)%>%select(-to.keep)
        # write.csv(ac,paste(Out.Dir,"Discard/multiplePSnonIdent.csv",sep=""))
        Counts<-Counts%>%full_join(ac%>%group_by(subtype.list)%>%summarise(multiplePSnonIdent=n()),by="subtype.list")
      
        # If the PPS and PS+1 are the same then keep a representative trucated to the PPS and PS+1
        ad<-ac%>%select(-dblPPS,-UNQ.PS.hits,-shared.protospacer.num,-protospacer.freq.num)%>%filter(spacer.order.number==1 | spacer.order.number==2)
        ...
        # Select the hits that are part of duplicated host genomes.
          duplicate.host.genomes <- ad%>%filter(duplicate.genomes != 1)
          #Counts<-Counts%>%full_join(duplicate.host.genomes %>%group_by(subtype.list)%>%summarise(IdentHostsHits=n()),by="subtype.list")
          #write.csv(duplicate.host.genomes,paste(Out.Dir,"Discard/duplicatehostgenomes.csv",sep=""))
...
# Now we need to select representative entries for each merged host genome set
  ...
  # Now try the representatives with the highest total bitscores scores
      ac<-duplicate.host.genomes%>%group_by(host.target.pair)%>%mutate(score.sum=sum(bit.score.num))
      ad<-ac%>%ungroup()%>%group_by(matching.host.genomes)%>%mutate(to.keep=ifelse(score.sum==max(score.sum),1,0))
      ad%>%group_by(to.keep)%>%summarise(counts=n())
      ae<-ad%>%filter(to.keep==1)
      #removed.dup.host.genomes<-removed.dup.host.genomes%>%bind_rows(ad%>%filter(to.keep==0))
  ...
  # Relabel the host genome column with the matching.host.genomes column and the the host.target.pair column #!!!! some of this needs moving to eariler
      ah<-ah%>%ungroup()%>%
        ...
        mutate(array.id=paste(unlist(list(unique(array.id))), collapse = "$merged_"))%>%
        #mutate(subtype.list=merged.subtypes)%>%
      select(-array.length,-shortest.array,-matching.host.genomes, -unique.protospacer.host.match, -all.target.PS,-score.sum,-represent,-to.keep)

      duplicate.host.genomes.represent <- unique(ah) ##!!!!! Print and check this table
      Counts<-Counts%>%full_join(duplicate.host.genomes.represent%>%group_by(subtype.list)%>%summarise(DupHost_PPS_PS1_RepresentKept=n()),by="subtype.list")

        
      
#*** Keep all the data that passed the host shared PS/redundancy filters (could possible add more here??!!)
      swipeData <- no.duplicate.PS%>%bind_rows(PPS.duplicate.spacer)%>%bind_rows(duplicate.host.genomes.represent)
...
#** Now do the filtering !! These steps might need checking!!??
  ...
  # Seperate out the identical targets then select a representative for these.
      ...
      no.IdentTargets<-dup.targets%>%filter(num.ident.targets==1)
      
      # Write out the counts
        Counts<-Counts%>%full_join(IdentTargets%>%group_by(subtype.list)%>%summarise(Dup_IdentTargets=n()),by="subtype.list")
        Counts<-Counts%>%full_join(no.IdentTargets%>%group_by(subtype.list)%>%summarise(Dup_noIdentTargets=n()),by="subtype.list")
    ...
    #Keep the highest total bitscore representatives 
      ...
      IdentTargetSubset<-IdentTargets%>%filter(to.keep==1)%>%ungroup()
      
  # Remove low bitscore then keep phage isolates if there are any
        IdentTargetSubset<-IdentTargetSubset%>%mutate(isolate=ifelse(grepl("-",target.acc.)==T,0,1))%>%
        group_by(UNQ.PS.pattern)%>%mutate(to.keep=ifelse(isolate==max(isolate),1,0))
        
        IdentTargetSubset<-IdentTargetSubset%>%filter(to.keep==1)
...
#*** Join back with the main non-perfect match duplicated data
  dup.targets<-no.IdentTargets%>%bind_rows(IdentTargets.Rep)
...
# Remake the spacer ID?
  ...
  host.target.pair.summary<-dup.targets%>%group_by(host.acc.,host.target.pair)%>%summarise(UNQ.host.spacers=paste(unlist(list(spacer.id)), collapse = ","))
...
# Add the number of times each of the host genomes occurs to the data set containing the host.target.pair data.
  host.target.pair.summary <- host.target.pair.summary%>%left_join(number.of.dup.targets, by = "UNQ.host.spacers")%>%ungroup()%>%select(-host.acc.,-UNQ.host.spacers)
...
#Tidy up the data that are still duplicated targets 
      still.dup.targets<-dup.targets%>%filter(duplicate.targets>1)
      Counts<-Counts%>%full_join(still.dup.targets%>%group_by(subtype.list)%>%summarise(Still.Dup.Data=n()),by="subtype.list")

    # It looks like most of what is left has either 2 or 3 duplicate targets...
        still.dup.targets%>%filter(spacer.order.number==1)%>%group_by(duplicate.targets)%>%summarise(counts=n())
    ...
    # If there is already a merged entry for lots of targets then ignore the outlier(s)
        ...
        ab<-aa%>%group_by(UNQ.PS.pattern)%>%mutate(to.keep=ifelse(Targets.Merged.Total>=5,1,0))
    ...
    # Keep these representatives
      ...
      # Check if each target has the same number of spacer matches
        ...
        ac<-ac%>%select(-UNQ.PS.ID,-UNQ.spacerID.pattern,-UNQ.PS.pattern,-spacer.id,-duplicate.genomes) ## Need to remove these from eariler in the script??!!
...
Counts<-Counts%>%full_join(non.duplicate.targets%>%group_by(subtype.list)%>%summarise(All.Data.Total=n()))
...
write.table(swipeData, "refseq_83.swipe.nr_17-4-18.txt", sep = "\t", quote = F, row.names = F, col.names = T)
...
swipeData <- read.table("refseq_83.swipe.nr_17-4-18.txt", comment.char = "", fill = T, sep = "\t", header = T,as.is=T)
...
ks_test_analysis(swipeData = swipeData, rh = rh, Subtype.label = "I-F")
...
for(i in subtypes){
quadrant_analysis(dat = swipeData, Subtype.label = i, use.hits = 2, window.width = 250000, run.statistical.test = F, write.data = F)
}
