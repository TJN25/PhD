...
# Now apply a higher bitsocre cut off for non PPS data
      LowScoreHits<-Data%>%filter(spacer.order.number>1 & bit.score.num<23)
      Counts<-Counts%>%full_join(LowScoreHits%>%group_by(subtype.list)%>%summarise(Low.BitScore.BeyondPPS.rem=n()),by="subtype.list")
      Data<-Data%>%filter(spacer.order.number==1 | bit.score.num>=23)
  ...
  # Now remove cases with the PS+1 and beyond that are low
      Data<-Data%>%mutate(low.score = ifelse(spacer.order.number!=1, ifelse(bit.score.num <= 24, 1, 0),0)) #
...
# Relative age based on position within array? prob only ok when there are just 2 hits # 10 added to array length to mitigate short array effects
    ...
    Data<-Data%>%mutate(PPS.score=ifelse(PPS.score!=0,PPS.score,ifelse(hits.count==2 & PPS.score1==1,1,0)))
...
# Keep everything with a PPS score >=1 #@@ need to write the removed data
    Data<-Data%>%filter(PPS.score>=1) #@@ changed for now although I think at == 5 is higher quality data.
...
#************************************************************************************************************************************************
 ...
 # Create unique PS ID containing the target position, accession and the spacer order number 
  ...
  # Select the hits that are part of duplicated host genomes.
        duplicate.host.genomes<-Data%>%filter(duplicate.genomes != 1)
...
# Now we need to select representative entries for each merged host genome set
  ...
  # First select the representatives with the highest PPS scores
      aa<-duplicate.host.genomes%>%group_by(matching.host.genomes)%>%mutate(to.keep=ifelse(PPS.score==max(PPS.score),1,0))
  ...
  # Combine all data back then further analysis will be done to check these are not similar enough to be called duplicates.
      rep.host.genomes<-non.duplicate.hosts%>%bind_rows(ident.host.genomes.represent)
...
#*** Keep all the data that passed the host shared PS/redundancy filters (could possible add more here??!!)
  ...
  aa<-Data%>%mutate(possible.crispr=ifelse(dist.from.PPS<=100 & dist.from.PPS>=1,1,0))%>%
  ...
  # Keep the good data
      Data<-aa%>%filter(poss.crispr==0)%>%select(-poss.crispr,-possible.crispr) 
  ...
  # The same spacer shouldn't target a phage twice
       aa<-Data%>%group_by(host.target.pair)%>%mutate(dup.spacers=ifelse(length(unique(spacer.number))==hits.count,0,1))
      ...
      # Keep the good data
        Data<-aa%>%filter(dup.spacers==0)%>%select(-dup.spacers) 
...
#** Now do the filtering !! These steps might need checking!!??
    ...
    # Select the duplicated data
        dup.targets<-Data%>%filter(duplicate.targets > 1)
...
# Join the now confirmed non-duplicated data back to the main table
  ...
  # Make a new UNQ.SpacerID.PS.ID with the distance to PPS rounded up to the nearest 1000.
      dup.targets<-still.dup.targets%>%ungroup()%>%mutate(round.dist.to.PPS=ceiling(dist.from.PPS/1000)*1000)
...
## Redo the close target with modifed rounding
        ...
        # Make a new UNQ.SpacerID.PS.ID with the distance to PPS rounded up to the nearest 1000.
      dup.targets<-dup.targets.out%>%ungroup()%>%mutate(round.dist.to.PPS=ceiling((dist.from.PPS-500)/1000)*1000)
      dup.targets<-dup.targets%>%mutate(UNQ.SpacerID.PS.ID=paste(spacer.id,round.dist.to.PPS,target.strand,sep = "_"))%>%
        group_by(host.target.pair)%>%mutate(UNQ.array.PS.round.pattern=paste(unlist(list(UNQ.SpacerID.PS.ID)), collapse = ","))
...
Final.Data<-Final.Data%>%filter(abs(dist.from.PPS)<=5000)

Quad.Data.All.Clust<-quadrant_analysis(Final.Data,"I-B", use.all.hits = T)%>%rename("freq_I-B"=Freq,"percent_I-B"=percentage)
Quad.Data.All.Clust<-Quad.Data.All.Clust%>%left_join(quadrant_analysis(Final.Data,"I-C", use.all.hits = T)%>%rename("freq_I-C"=Freq,"percent_I-C"=percentage))
Quad.Data.All.Clust<-Quad.Data.All.Clust%>%left_join(quadrant_analysis(Final.Data,"I-E", use.all.hits = T)%>%rename("freq_I-E"=Freq,"percent_I-E"=percentage))
Quad.Data.All.Clust<-Quad.Data.All.Clust%>%left_join(quadrant_analysis(Final.Data,"I-F", use.all.hits = T)%>%rename("freq_I-F"=Freq,"percent_I-F"=percentage))
Quad.Data.All.Clust<-Quad.Data.All.Clust%>%left_join(quadrant_analysis(Final.Data,"II-A", use.all.hits = T)%>%rename("freq_II-A"=Freq,"percent_II-A"=percentage))
Quad.Data.All.Clust<-Quad.Data.All.Clust%>%left_join(quadrant_analysis(Final.Data,"II-C", use.all.hits = T)%>%rename("freq_II-C"=Freq,"percent_II-C"=percentage))

write.csv(Quad.Data.All.Clust,paste(Out.Dir,"Graph_QuadDataAllClust.csv",sep=""))

Quad.Data.PS1PS2.Clust<-quadrant_analysis(Final.Data,"I-B", use.all.hits = F)%>%rename("freq_I-B"=Freq,"percent_I-B"=percentage)
Quad.Data.PS1PS2.Clust<-Quad.Data.PS1PS2.Clust%>%left_join(quadrant_analysis(Final.Data,"I-C", use.all.hits = F)%>%rename("freq_I-C"=Freq,"percent_I-C"=percentage))
Quad.Data.PS1PS2.Clust<-Quad.Data.PS1PS2.Clust%>%left_join(quadrant_analysis(Final.Data,"I-E", use.all.hits = F)%>%rename("freq_I-E"=Freq,"percent_I-E"=percentage))
Quad.Data.PS1PS2.Clust<-Quad.Data.PS1PS2.Clust%>%left_join(quadrant_analysis(Final.Data,"I-F", use.all.hits = F)%>%rename("freq_I-F"=Freq,"percent_I-F"=percentage))
Quad.Data.PS1PS2.Clust<-Quad.Data.PS1PS2.Clust%>%left_join(quadrant_analysis(Final.Data,"II-A", use.all.hits = F)%>%rename("freq_II-A"=Freq,"percent_II-A"=percentage))
Quad.Data.PS1PS2.Clust<-Quad.Data.PS1PS2.Clust%>%left_join(quadrant_analysis(Final.Data,"II-C", use.all.hits = F)%>%rename("freq_II-C"=Freq,"percent_II-C"=percentage))
