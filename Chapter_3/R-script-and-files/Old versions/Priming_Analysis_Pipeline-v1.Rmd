...
rRepeat <- repeatsFound%>%group_by(array.id)%>%mutate(spacer.num.corrected= rev(spacer.num))
cRepeat <- repeatsFound%>%mutate(spacer.num.corrected= spacer.num)

allRepeats <- rRepeat%>%bind_rows(cRepeat)

allRepeats <- allRepeats%>%mutate(spacer.id = paste(array.id, spacer.num, sep = "-"))%>%ungroup()%>%select(spacer.id, Direction, spacer.num.corrected)
...
swipeDataFixed <- swipeDataFixed%>%filter(!is.na(Direction))%>%
...
#####
swipeData <-  read.table("refseq_83.swipe.nr.clustered_27-03-18.txt", comment.char = "", fill = T, sep = "\t", header = T)
...
IFData <- swipeData%>%filter(Subtype == "I-C")%>%filter(spacer.order.number != 1)
clusteredData <- swipeData
...
Writes files for Prism (NOT WORKING)
```{r analysis_output}
##input variables
  Subtype.label <- "I-F"
  binwidth <- 150
...
##UP TO HERE  ###### 
    ## Normalises the distrbution values #####
...
######
 ...
 write.csv(hits, file = paste("Project/Prism_tables/", Subtype.label, "_standard.csv", sep = ""), row.names = FALSE)
