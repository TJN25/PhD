...
# Import the final dataset
    Final.Data<-read.csv("FinalData.csv", as.is=T)

##number of host-target pairs
Final.Data%>%group_by(Subtype)%>% filter(spacer.order.number == 1)%>%summarise(x = n())

##number of protospacers
Final.Data%>%group_by(Subtype)%>%summarise(x = n())
...
to.map.narrow <- to.map%>%filter(dist.from.PPS >= -10000, dist.from.PPS <= 10000)
...
rh.narrow<-generate_random_distribution(to.map.narrow)


for(i in subtypes){ # Duplicate 0 in row names messes this up - why are we splitting into quadrants?
  den<-protospacer_distribution(swipeData = to.map, rh = rh, Subtype.label = i)
  hits<-prism_format_protospacer_distribution(den = den)
  hits<-normalise_distribution_values(hits)
  hits<-hits%>%rename(!! paste(i, "_t") := target.density.values)%>%
                rename(!! paste(i, "_nt") := non.target.density.values)%>%
                rename(!! paste(i, "_rnd_t") := random.mean.t.density.values)%>%
                rename(!! paste(i, "_rnd_nt") := random.mean.n.density.values)
 

 if (i == "I-B"){PS.maps<-hits} else {PS.maps<-PS.maps%>%left_join(hits,by="distance.breaks.short")}

 }

for(i in subtypes){
  x<-ks_test_analysis(swipeData = to.map.narrow, rh = rh.narrow, Subtype.label = i)
...
row.null.neg<-PS.maps%>%filter(distance.breaks.short==mapping.size*-1)
row.null.neg[2:ncol(row.null.neg)]<-0
row.null.pos<-PS.maps%>%filter(distance.breaks.short==mapping.size)
...
write.csv(PS.maps,paste("Output/Analysis/Protospacer_Mapping_All_BW-",binwidth,".csv",sep=""), row.names = F)
