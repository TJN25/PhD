...
setwd("~/Brown_Lab/R-script-and-files") #!!! I have added !!! to mark any changes
Out.Dir<-"Output/"
...
Counts<-swipeData%>%group_by(subtype.list)%>%summarise(RawSwipeData=n()) #!!! 
...
removed<-swipeData%>%filter(to.remove>1)

removed<-removed%>%group_by(host.target.pair)%>%mutate(hits.count=n())

write.csv(removed,paste(Out.Dir,"Discard/Removed.csv",sep=""))

rm(removed)

swipeData <- swipeData%>%filter(to.remove == 1)%>%
  select(-duplicate.spacer.order, -duplicate.spacers,-to.remove)
...
##THIS IS CHANGING THE SUBTYPE TO MATCH THE CALL FROM THE REPEATS !!! removed this
...
#!!! added this
repeatsNotFound <- repeatsFound%>%filter(subtype.correct != 1)

repeatsFound <- repeatsFound%>%filter(subtype.count == 1)%>%select(-subtype.count)


write.csv(repeatsNotFound ,paste(Out.Dir,"Discard/SubtypeDontMatch.csv",sep=""))
...
Counts<-Counts%>%full_join(swipeData%>%group_by(subtype.list)%>%summarise(Swipe_Direction_Corrected=n()),by="subtype.list") #!!!
...
Counts<-Counts%>%full_join(swipeData%>%group_by(subtype.list)%>%summarise(Host.Target.Pair.Setup=n()),by="subtype.list") #!!!
...
Counts<-Counts%>%full_join(swipeData%>%group_by(subtype.list)%>%summarise(Swipe.PPS.Scores=n()),by="subtype.list") #!!!
...
Counts<-Counts%>%full_join(swipeData%>%group_by(subtype.list)%>%summarise(BitScore.Filtered=n()),by="subtype.list") #!!!


swipeData <- swipeData%>%group_by(host.target.pair)%>%mutate(hits.count = n())%>%filter(hits.count > 1)

Counts<-Counts%>%full_join(swipeData%>%group_by(subtype.list)%>%summarise(TwoHits.Filtered=n()),by="subtype.list") #!!!
...
Counts<-Counts%>%full_join(one.host.genome%>%group_by(subtype.list)%>%summarise(Genomes.Merged=n()),by="subtype.list") #!!!
...
Counts<-Counts%>%full_join(swipeData%>%group_by(subtype.list)%>%summarise(Swipe.Hosts.nr=n()),by="subtype.list") #!!!
...
Counts<-Counts%>%full_join(swipeData%>%mutate(subtype.list=Subtype)%>%group_by(subtype.list)%>%summarise(Swipe.nr2=n()),by="subtype.list") #!!!
...
Counts<-Counts%>%full_join(swipeData%>%group_by(subtype.list)%>%summarise(counts3=n()),by="subtype.list") #!!!
...
Counts<-Counts%>%full_join(swipeData%>%mutate(subtype.list=Subtype)%>%group_by(subtype.list)%>%summarise(SwipeData4=n()),by="subtype.list") #!!!

Counts<-Counts%>%full_join(clusteredData%>%mutate(subtype.list=Subtype)%>%group_by(subtype.list)%>%summarise(ClusteredData4=n()),by="subtype.list") #!!!
