...
spacers <-  read.table("two_or_more_hits_repeat_file_to_work_with.txt", comment.char = "", fill = T, sep = "\t", header = T, quote = "", as.is = T)
repeats <-  read.csv("New_Lookup_SJ-27-03-18-v4.csv", comment.char = "", fill = T, header = T, quote = "", as.is = T)



tmp2 <- unique(spacers%>%select(contig, Organism, host.acc.))

tmp2 <- tmp2%>%mutate(x = rep(F, nrow(tmp2)))
for(i in 1:nrow(tmp2)){
print(i)
    tmp2[i, 4] <- grepl(tmp2[i,1], tmp2[i,3])
}

tmp <- swipeData%>%filter(subtype.list == "I-C")
...
tmp <- swipeDataFixed%>%filter(subtype.list == "I-C")
...
spacers <- left_join(spacers, repeatData, by = "Primary_Repeat")

tmp <- cas_genes%>%filter(grepl("type ", name))
mat <- tidyr::separate(tmp, name,c("i1","i2"), sep = "type ", remove = F, extra = "merge")
mat <- tidyr::separate(mat, i2,c("subtype","i3"), sep = " ", remove = F, extra = "merge")

mat <- mat%>%
  filter(grepl("I", subtype) | grepl("V", subtype))%>%
  filter(subtype != "III-associated")%>%
  filter(subtype != "I" & subtype != "II" & subtype != "III")%>%
  filter(grepl("M", subtype) == F)%>%
  filter(grepl("P", subtype) == F)

tmp <- mat%>%group_by(assembly)%>%summarise(subtypes.found = paste(unique(sort(subtype)), collapse = ","))
subtype.counts <- mat%>%group_by(assembly)%>%summarise(subtypes.count = length(unique(subtype)))

tmp <- left_join(tmp, subtype.counts, by = "assembly")

tmp <- tmp%>%dplyr::rename(assembly_accession = assembly)

repeatsFound <- spacers%>%filter(!is.na(as.character(Direction)))%>%
...
repeatsOther <- spacers%>%filter(is.na(as.character(Direction)))

repeatsOther <- repeatsOther%>%
  mutate(keep.genome = rep(0, nrow(repeatsOther)))

tmp2 <- left_join(repeatsFound, tmp, by = "assembly_accession")




subtypes <- c("I-A", "I-B", "I-C", "I-D", "I-E", "I-F", "I-U", "II-A", "II-B", "II-C", "III-A", "III-B", "III-C", "III-D")
tmp3 <- tmp2%>%filter(!is.na(subtypes.found))%>%
  mutate(array.correct = 0)%>%
  mutate(gene.correct = 0)
tmp4 <- tmp2%>%filter(is.na(subtypes.found))
for(i in subtypes){
    if(i != "II-A" & i != "II-B" & i != "II-C"){
      tmp3 <- tmp3%>%mutate(array.correct = ifelse(i == Subtype & grepl(i, subtypes.found), 1, array.correct))%>%
      mutate(gene.correct = ifelse(i == subtype.list & grepl(i, subtypes.found), 1, gene.correct))
    }else{
      tmp3 <- tmp3%>%mutate(array.correct = ifelse("II" == Subtype & grepl("II", subtypes.found), 1, array.correct))%>%
      mutate(gene.correct = ifelse(i == subtype.list & grepl(i, subtypes.found), 1, gene.correct))
    }
...
tmp3 <- tmp3%>%
  mutate(keep.genome = ifelse(Subtype == subtype.list, 1, 0))%>%
  mutate(keep.genome = ifelse(Subtype == "II" & subtypes.count == 1, ifelse(subtype.list == "II-A" | subtype.list == "II-B" | subtype.list == "II-C", 1, keep.genome), keep.genome))%>%
  mutate(keep.genome = ifelse(array.correct == 1 & subtypes.count == 1 |gene.correct == 1 & subtypes.count == 1, 1, keep.genome))




genomesToKeep <- tmp3%>%filter(keep.genome == 1)

genomesToRemove <- tmp3%>%filter(keep.genome != 1)

genomesOther <- tmp4%>%
  mutate(array.correct = rep(2, nrow(tmp4)))%>%
  mutate(gene.correct = rep(2, nrow(tmp4)))%>%
  mutate(keep.genome = rep(2, nrow(tmp4)))


genomesToKeep <- genomesToKeep%>%bind_rows(genomesOther)

genomesToRemove2 <- genomesToKeep%>%bind_rows(genomesOther)%>%
  filter(subtype.correct == 0 & keep.genome == 2)


genomesToRemove <- genomesToRemove%>%bind_rows(genomesToRemove2)

genomesToRemove <- genomesToRemove%>%mutate(keep.genome = rep(0, nrow(genomesToRemove)))

genomesToKeep <- genomesToKeep%>%
  filter(subtype.correct == 1 | keep.genome == 1)


genomesToKeep <- unique(genomesToKeep)

genomesToKeep <- genomesToKeep%>%mutate(subtype.list = ifelse(array.correct == 1, ifelse(gene.correct == 0, subtypes.found, subtype.list), subtype.list))

genomesToKeep <- genomesToKeep%>%bind_rows(genomesToRemove)

genomesToKeep <- genomesToKeep%>%select(-subtype.correct, -subtypes.found, -subtypes.count, -array.correct, -gene.correct)

genomesToKeep <- genomesToKeep%>%bind_rows(repeatsOther)
...
allGenomes <- allGenomes%>%mutate(array.id = paste(contig, array.num, sep = "-"))



cRepeat <- allGenomes%>%filter(Direction != "ReverseComplement")
rRepeat <- allGenomes%>%filter(Direction == "ReverseComplement")
...
allGenomes <- rRepeat%>%bind_rows(cRepeat)



allGenomes <- allGenomes%>%ungroup()%>%mutate(spacer.id = paste(array.id, spacer.num, sep = "-"))%>%select(spacer.id, Direction, Subtype, keep.genome, spacer.num.corrected)

allGenomes <- unique(allGenomes)
...
swipeDataFixed <- swipeData%>%mutate(spacer.id = paste(array.id, spacer.number, sep = "-"))
swipeDataFixed <- left_join(swipeDataFixed, allGenomes, by = "spacer.id")

swipeDataFixed <- as.data.frame(swipeDataFixed)
...
swipeDataFixed <- swipeDataFixed%>%mutate(keep.genome = ifelse(is.na(keep.genome), 1, keep.genome))%>%filter(keep.genome != 0)%>%
  mutate(spacer.num.corrected = ifelse(is.na(spacer.num.corrected), spacer.number, spacer.num.corrected))%>%
  mutate(spacer.number = spacer.num.corrected)%>%
  mutate(strand = ifelse(is.na(Direction), strand, ifelse(Direction == "ReverseComplement", -1*as.numeric(strand), as.numeric(strand))))
...
swipeData <- swipeDataFixed%>%select(-spacer.id, -Direction, -spacer.num.corrected, - keep.genome, -Subtype)
...
tmp2 <- unique(swipeData%>%select(host.acc., assembly_accession, spacer.acc.))

tmp2 <- tmp2%>%mutate(x = rep(F, nrow(tmp2)))
for(i in 1:nrow(tmp2)){
print(i)
 try(   tmp2[i, 4] <- grepl(tmp2[i,1], tmp2[i,3]))
}
