...
#**** Split the spacers data depending on whether the repeat was classified in my file
  ...
  #!!?? I am worried that we are keeping 'bad' data by doing this...
    ...
    all.GCFs <- swipeData%>%ungroup()%>%select(assembly_accession,subtype.list)%>%unique()
    spacers.plus.all.GCFs<-spacers%>%full_join(all.GCFs)

  tmp2 <- spacers.plus.all.GCFs%>%left_join(tmp, by = "assembly_accession")

    # If the subtypes.count or direction columns are na then just set to 1
...
aaa<-tmp2%>%group_by(subtypes.genes.found)%>%summarise(freq=n())
    ...
    Counts<-Counts%>%full_join(tmp2%>%group_by(subtype.list)%>%summarise(reps.AllSystems.total=n()),by="subtype.list")
...
#First remove cases where more than one subtype are called in my cas gene call table
  ...
  # Keep the good data with gene for only one subtype
    single.subtype.genes<-tmp2%>%filter(subtypes.count==1)
...
# Now remove cases where the repeat family and subtype call don't match
    # First fill in the na subtype list - if we didn't change the repeat, keep as is.
      single.subtype.genes<-single.subtype.genes%>%mutate(Subtype=ifelse(is.na(Subtype),subtype.list,Subtype)) 
      single.subtype.genes<-single.subtype.genes%>%mutate(subtypes.genes.found=ifelse(is.na(subtypes.genes.found),subtype.list,subtypes.genes.found)) 
    # Checks
      table(single.subtype.genes$Subtype)
      table(single.subtype.genes$subtype.list)
      table(single.subtype.genes$subtypes.genes.found)
    
    
  # Now update to keep column ##!!?? need to fix up the type II systems.. as per the genes in subtypes fix above.
    ab<-single.subtype.genes%>%mutate(keep.genome=ifelse(subtype.list==Subtype & subtype.list==subtypes.genes.found,1,ifelse(Subtype=="II", ifelse(subtype.list=="II-A"|subtype.list=="II-B"|subtype.list=="II-C",1,0),0)))  
    # Check
      table(ab$keep.genome)
    
  # Write out the removed data to look at it
    removed<-ab%>%filter(keep.genome==0)
    Counts<-Counts%>%full_join(removed%>%group_by(subtype.list)%>%summarise(reps.InconsistentSystems.rem=n()),by="subtype.list")
...
# Keep the systems with consistent subtype calls
    ...
    Counts<-Counts%>%full_join(genomesToKeep%>%group_by(subtype.list)%>%summarise(reps.PassedSystems.kept=n()),by="subtype.list")

# Still have to fix the cases where they weren't in the spacer file... #This seperates out the GCFs not in the spacer file so they can be added back later
    GCFs.not.in.spacer.file.to.keep<-genomesToKeep%>%filter(is.na(Organism))
...
# Correcting the spacer order for backwards arrays (based on repeat family)
    cRepeat<-genomesToKeep%>%filter(Direction !="ReverseComplement")%>%mutate(spacer.num.corrected=spacer.num)
    rRepeat<-genomesToKeep%>%filter(Direction =="ReverseComplement")%>%group_by(array.id)%>%mutate(spacer.num.corrected=rev(spacer.num))
  ...
  length(genomesToKeep)-length(unique(genomesToKeep)) ##!! Why do we lose data here? - problem in spacer flie - have implemented a fix for this
...
#************************************************************************************************************************************************
  ...
  # First just apply a higher bitsocre cut off for all data
    swipeData <- swipeData%>%filter(bit.score.num>=21)
  
    LowScoreHits <- swipeData%>%filter(bit.score.num<21)
