...
length(swipeData)-length(swipeData.In) # Check
...
mat%>%group_by(gene.subtype)%>%summarise(freq=n())
...
# Collate the system calls and count the number of systems per host GCF
    NCBI.system.calls<-mat%>%group_by(assembly)%>%
        summarise(subtypes.count = length(unique(gene.subtype)),subtypes.genes.found = paste(unique(sort(gene.subtype)), collapse = ","))%>%
...
# Join the cas gene calls and repeat type calls to the swipe data
  ...
  length(Main.Data)-length(Main.Data%>%unique()) # Check this ==0
...
# Now remove cases where the repeat type is set to "Repeat_Fail" -> can't just discard arrays, it needs to be the whole host-target pair.
  ...
  # Keep the good data
        Main.Data<-Main.Data%>%filter(to.remove==0)%>%select(-to.remove)
...
# Now remove cases where the repeat family and subtype calls don't match
  ...
  # Now check if all the assignements match up
    ...
    # Fix up the type II systems 
        Main.Data<-Main.Data%>%
                mutate(to.keep=ifelse(Repeat.Subtype!="II",to.keep, ifelse(subtype.list=="II-A"|subtype.list=="II-B"|subtype.list=="II-C",1,0)))  
...
write.csv(Main.Data,paste(Out.Dir,"DataCorrected.csv",sep=""),row.names=F)
...
Data<-read.csv(paste(Out.Dir,"DataCorrected.csv",sep=""),as.is=T)
...
# Now apply a higher bitscore cut off for non PPS data
      LowScoreHits<-Data%>%filter(spacer.order.number>1 & bit.score.num<21)
      Counts<-Counts%>%full_join(LowScoreHits%>%group_by(subtype.list)%>%summarise(Low.BitScore.BeyondPPS.rem=n()),by="subtype.list")
      Data<-Data%>%filter(spacer.order.number==1 | bit.score.num>=21)
...
write.csv(Data,paste(Out.Dir,"Bitscore_Filtered.csv",sep=""),row.names=F)
...
Data<-read.csv(paste(Out.Dir,"Bitscore_Filtered.csv",sep=""),as.is=T)
...
write.csv(Data,paste(Out.Dir,"PPS_Distances_Added.csv",sep=""),row.names=F)
...
Data<-read.csv(paste(Out.Dir,"PPS_Distances_Added.csv",sep=""),as.is=T)
...
# If the two oldest spacers are from the same array AND this is the shortest array by at least 10?, then set to 4? ## Doesn't allow many hits through
    Data<-Data%>%group_by(host.target.pair)%>%mutate(PPS.array.shortest=ifelse(spacer.order.number==1 & array.length<max(array.length)-10,1,0))%>%
...
# Keep everything with a PPS score >=1 #@@ need to write the removed data
    Data<-Data%>%filter(PPS.score>=5) #@@ changed for now although I think at == 5 is higher quality data.
...
write.csv(Data,paste(Out.Dir,"PPS_Scores_Filtered.csv",sep=""),row.names=F)
...
Data<-read.csv(paste(Out.Dir,"PPS_Scores_Filtered.csv",sep=""),as.is=T)
...
# It looks like there might be some CRISPR arrays in there - filter for short distances to PPS?
  ...
  # Remove some columns
      Data<-Data%>%select(-protospacer.freq.num,-array.length,-total.shared.protospacers,-UNQ.protospacer.ID,-duplicate.genomes)
      Data%>%group_by(host.target.pair)%>%summarise(hit.count=n())%>%group_by(hit.count)%>%summarise(freq=n())

      Counts<-Counts%>%full_join(Data%>%rename(subtype.list = Subtype)%>%group_by(subtype.list)%>%summarise(PostHostFilterAllHits.total=n()),by="subtype.list")

write.csv(Data,paste(Out.Dir,"Data_Clean_Out.csv",sep=""),row.names=F)
...
Data<-read.csv(paste(Out.Dir,"Data_Clean_Out.csv",sep=""),as.is=T)

# Check host-target pairs have at least 2 hits
  Data<-Data%>%group_by(host.target.pair)%>%mutate(hits.count=n()) 
  table(Data$hits.count) # Check this

  Counts<-Counts%>%full_join(Data%>%group_by(subtype.list)%>%summarise(RedundancyIn.total=n()),by="subtype.list")
...
#************************************************************************************************************************************************
 ...
 # Create unique PS ID containing the target position, accession and the spacer order number 
  ...
  # Combine each of the unique.protospacer.host.matches for each of the host target pairs so that the entire match is summarised in one column.
      host.target.pair.summary<-Data%>%group_by(target.acc.,host.target.pair)%>%summarise(all.target.PS=paste(unlist(list(unique.protospacer.host.match)),collapse=","))
  ...
  # Add the duplicate genomes data to the hits data
      ...
      # Count non identical dup data
        ...
        Counts<-Counts%>%full_join(non.duplicate.hosts %>%group_by(subtype.list)%>%summarise(nonIdentHostsHits.subtot=n()),by="subtype.list")
  
      # Select the hits that are part of duplicated host genomes.
          hosts.to.merge<-Data%>%filter(duplicate.genomes != 1)
          Counts<-Counts%>%full_join(hosts.to.merge%>%group_by(subtype.list)%>%summarise(IdentHostsHits.subtot=n()),by="subtype.list")
...
#************************************************************************************************************************************************
   ...
   # Add unique hit information to a column 
        Data<-Data%>%ungroup()%>%mutate(UNQ.protospacer.ID =  paste(target.acc., target.pos, sep = "_"))

    # Count the number of times a protospacer site is matched
        protospacer.counts.dat<-Data%>%group_by(target.acc., UNQ.protospacer.ID)%>%
          summarise(protospacer.freq.num = n())%>%ungroup()%>%select(-target.acc.)

    # Add the protospacer counts data
        Data<-Data%>%left_join(protospacer.counts.dat, by = "UNQ.protospacer.ID")
    ...
    # Count the duplicated PS - e.g. if there are 7 PS for a host-target, how many of the 7 PS are shared with other host-target pairs)
        PS.counts.dat<-Data%>%filter(protospacer.freq.num > 1)%>%group_by(host.target.pair)%>%summarise(total.shared.protospacers = n())

    # Add the spacer counts data
        Data<-Data%>%left_join(PS.counts.dat, by ="host.target.pair")
        Data$total.shared.protospacers[is.na(Data$total.shared.protospacers)]<-0

  # Data checks
      Counts<-Counts%>%full_join(Data%>%group_by(subtype.list)%>%summarise(PostIdentGen.total=n()),by="subtype.list")
      write.csv(Data,paste(Out.Dir,"AllHostGenome.csv",sep=""))

  # Keep the host-target pairs with no duplicate spacers/PS
      no.duplicate.PS<-Data%>%filter(total.shared.protospacers==0)
      Counts<-Counts%>%full_join(no.duplicate.PS%>%group_by(subtype.list)%>%summarise(NoSharedPS.kept=n()),by="subtype.list")
      Counts<-Counts%>%full_join(Data%>%filter(total.shared.protospacers>0)%>%group_by(subtype.list)%>%summarise(HasSharedPS.total=n()),by="subtype.list")
  ...
  # Seperate the host-target pairs with only a single shared protospacer site (this should look at only the cases where the PPS is duplicated initially)
      single.duplicate.spacer<-Data%>%filter(total.shared.protospacers == 1)
  ...
  # Seperate any host-target pairs with more than 1 shared spacer on the same target
        many.shared.PS<-Data%>%filter(total.shared.protospacers > 1)
      ...
      # Check if the PPS is found twice
          multiplePPS<-many.shared.PS%>%filter(spacer.order.number!=1)%>%group_by(host.target.pair)%>%
              mutate(doublePPS=ifelse(dist.from.PPS==0,1,0))%>%group_by(host.target.pair)%>%summarise(dblPPS=max(doublePPS))
      ...
      # Check if there are duplicated PS within the same host-target pair
        ...
        # Count the number of times each of the host PS patterns are identical for each of the target genomes.
            num.duplicate.hosts<-host.target.pair.summary%>%group_by(target.acc.,UNQ.array.PS.pattern)%>%
        ...
        # Add the number of times each of the host PS patterns occurs to the data set containing the host.target.pair data.
            host.target.pair.summary<-host.target.pair.summary%>%left_join(num.duplicate.hosts, by = "UNQ.array.PS.pattern")%>%ungroup()%>%select(-target.acc.)
...
#*** Keep all the data that passed the host shared PS/redundancy filters (could possible add more here??!!)
      Data<-duplicate.host.genomes.represent%>%bind_rows(no.duplicate.PS)%>%bind_rows(PPS.duplicate.spacer) ## this is not working!!??
      
      Counts<-Counts%>%full_join(Data%>%group_by(subtype.list)%>%summarise(OutputHostRedundancyFilter.total=n()),by="subtype.list")
...
write.csv(Data,paste(Out.Dir,"Host_Redundancy_Filtered.csv",sep=""),row.names=F)
...
Data<-read.csv(paste(Out.Dir,"Host_Redundancy_Filtered.csv",sep=""),as.is=T)

# Check host-target pairs have at least 2 hits
  Data<-Data%>%group_by(host.target.pair)%>%mutate(hits.count=n()) 
  table(Data$hits.count) # Check this

  Counts<-Counts%>%full_join(Data%>%group_by(subtype.list)%>%summarise(TargetRedundancyIn.total=n()),by="subtype.list")
  Data%>%group_by(host.target.pair)%>%summarise(hit.count=n())%>%group_by(hit.count)%>%summarise(freq=n())
...
# Start by finding cases where the whole set of host spacers matching multiple targets are the same
  ...
  # Create a unique spacer ID containing the host array details, array, spacer number
      ...
      Data%>%group_by(spacer.id)%>%summarise(freq=n())%>%group_by(freq)%>%summarise(counts=n())

  # Combine each of the spacer.ids for each of the host target pairs to give a unique host spacer set ID
      host.target.pair.summary<-Data%>%group_by(host.acc.,host.target.pair)%>%  
  ...
  # Add the duplicate target/host spacer set info to the hits data
      Data<-left_join(Data, host.target.pair.summary, by = "host.target.pair")
      Data%>%group_by(duplicate.targets)%>%summarise(freq=n())
      Counts<-Counts%>%full_join(Data%>%group_by(subtype.list)%>%summarise(PreTargetFilters.total=n()),by="subtype.list")

#** Now do the filtering !! These steps might need checking!!??
    # Data OK as is for now
        non.duplicate.targets<-Data%>%filter(duplicate.targets == 1)%>%select(-duplicate.targets,-spacer.id,-UNQ.host.spacer.set)
...
#         
    ...
    # If there are only 2 duplicate targets and their quadrants are the same then average the distances (as long as they're less than 5 kb apart?**)@@ **not implmented yet
        merge.quads<-dup.targets%>%filter(UNQ.quads==1)%>%filter(duplicate.targets==2)%>%
  ...
  # Add to the running non duplicate data table    
    ...
    non.duplicate.targets<-non.duplicate.targets%>%bind_rows(merge.quads)
    Counts<-Counts%>%full_join(non.duplicate.targets%>%group_by(subtype.list)%>%summarise(CurrentNonDuplicate.total=n()),by="subtype.list")
    nrow(non.duplicate.targets)-nrow(non.duplicate.targets%>%unique()) # Check all rows are unique        
...
#   # Sort out the rest  
      ...
      Counts<-Counts%>%full_join(still.duplicate.PS%>%group_by(subtype.list)%>%summarise(FinalStillDuplicatePS.rem=n()),by="subtype.list")
  ...
  # If the strand and direction (quadrants) are the same then we can just average the distances
    ...
    # If there are only 2 duplicate targets and their quadrants are the same then average the distances (as long as they're less than 5 kb apart?**)@@ **not implmented yet
        merge.quads<-duplicate.PS%>%filter(UNQ.quads==1)%>%filter(total.targets==2)%>%
  ...
  # Add to the running non duplicate data table    
    ...
    Counts<-Counts%>%full_join(no.duplicate.PS%>%group_by(subtype.list)%>%summarise(CurrentNonDuplicate.total=n()),by="subtype.list")
    nrow(no.duplicate.PS)-nrow(no.duplicate.PS%>%unique()) # Check all rows are unique        
...
#  
# #     # If PPS, PS+1 are the same spacers, then truncate to just these
...
Data<-no.duplicate.PS%>%rename(Subtype = subtype.list)%>%select(-shared.spacer.num,-spacer.id,-spacer.freq.num,-pps.strand)

Counts<-Counts%>%full_join(Data%>%rename(subtype.list = Subtype)%>%group_by(subtype.list)%>%summarise(PostFilterAllHits.total=n()),by="subtype.list")
...
write.csv(Counts,paste(Out.Dir,"Data_Count_Table_SJ-v11.csv",sep=""))

write.csv(Data,paste(Out.Dir,"FinalData.csv",sep=""),row.names=F)
...
# Import the final dataset
    Final.Data<-read.csv(paste(Out.Dir,"FinalData.csv",sep=""),as.is=T)
