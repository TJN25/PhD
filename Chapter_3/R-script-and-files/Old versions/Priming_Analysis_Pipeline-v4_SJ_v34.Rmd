...
# It looks like there might be some CRISPR arrays in there - filter for short distances to PPS?
  ...
  # Two spacers from the same shouldn't target the smae phage PS (biologically possible, yet unlikely and causes problems later on)
...
#         Counts<-Counts%>%full_join(ac%>%group_by(subtype.list)%>%summarise(multiplePS_NoDuplicatedPS.kept=n()),by="subtype.list")
        ...
        # Combine each of the UNQ.protospacer.IDs for each of the host target pairs so that the entire match is summarised in one column.
            many.shared.PS.trunc<-many.shared.PS.trunc%>%group_by(host.target.pair)%>%mutate(UNQ.array.PS.pattern=paste(unlist(list(UNQ.protospacer.ID)),collapse=","))
        ...
        # Select the hits that are part of perfectly duplicated host genomes.
            # Write out the data to remove
                other.truc.hosts<-many.shared.PS.trunc%>%filter(ident.trunc.hosts==1)
            ...
            # Keep the good data
                trunc.hosts.to.merge<-many.shared.PS.trunc%>%filter(ident.trunc.hosts!=1)
...
#*****************************************************************************************************    
    ...
    # Just keep the non duplicated data for now....
      AllData<-non.duplicate.targets%>%ungroup()%>%
  ...
  # Keep the host-target pairs with no duplicate spacers
    ...
    Counts<-Counts%>%full_join(no.duplicate.PS%>%group_by(subtype.list)%>%summarise(NoDuplicatePS_D.kept=n()),by="subtype.list")
...
#   # Sort out the rest  
      ...
      Counts<-Counts%>%full_join(still.duplicate.PS%>%group_by(subtype.list)%>%summarise(StillDuplicatePS_D.subtot=n()),by="subtype.list")
  ...
  # Add to the running non duplicate data table    
    ...
    Counts<-Counts%>%full_join(no.duplicate.PS%>%group_by(subtype.list)%>%summarise(CurrentNonDuplicate_E.total=n()),by="subtype.list")
