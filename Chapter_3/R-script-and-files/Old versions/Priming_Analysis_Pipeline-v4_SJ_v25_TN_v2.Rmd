...
mat.res <- multinomial.test(observed = quadrants$Freq, prob = c(0.25, 0.25, 0.25, 0.25), MonteCarlo = T, ntrial = 10000) # I don't think the Monte Carlo is necessary for our (simple) system
##It is if we want it to run on a slow computer when there are lots of hits. My laptop doesn't handle the standard comand
# OK - we will need to run with more trials then, otherwise the p values are not reliable. Or I can run the non-simulated test for the final p values
...
Out.Table<-Out.Table%>%dplyr::rename("Var1"=value)
...
swipeData <- swipeData.In%>%filter(bit.score.num >=20)
...
swipeDataOld <-  read.table("refseq_83_swipe_array_direction_corrected_TN_v2.txt", comment.char = "", fill = T, sep = "\t", header = T, quote = "", as.is=T)
...
#************************************************************************************************************************************************
  ...
  # First just apply a higher bitsocre cut off for all data
    ...
    LowScoreHits <- swipeData%>%filter(bit.score.num<20)
  ...
  # Now remove cases with the PS+1 and beyond that are low
    ...
    swipeData <- swipeData%>%mutate(low.score = ifelse(spacer.order.number!=1, ifelse(bit.score.num <= 22, 1, 0),0)) ## fix this
...
swipeData%>%group_by(Subtype)%>%summarise(counts =n())
